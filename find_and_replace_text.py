...
Тест работоспособности проводился на двух машинах с Windows 10. С GUI на Windows XP не работает(скорей всего виновата 
сборка в исполняющий файл) """
...
def refind_in_paragraphs(doc_parag,text_find,text_refind,list_repl):  # поиск и замена в параграфе (или ячейки таблицы)
    ...
    for x in doc_parag.paragraphs:  # обходим параграфы текста
        if text_find in x.text:  # если есть, то выполняем замену.
            ...
            for para in list_of_changes:  # если список найденных попадает в список уже заменненых, то пропустить все замены
                if x.text == para[0]:
                    if replacement_positions[0] in range(para[1][0][0],para[1][0][1]):  # проверка была ли замена этого значения, если ДА, то пропустить
                        return None
            ...
            for i in range(len(inline)):  # разбираем построчно
                ...
                if text_find in inline[i].text:  # проверка каждого слова и замена всех совпадений
                    ...
                    text = inline[i].text.replace(text_find[ref_start:ref_stop],
                                                  text_refind[ref_start:ref_stop])  # создаем кусок под замену
                    inline[i].text = text  # заменяем кусок
                    # print("сделано в параграфах! " + text)

            substitution_values.append(x.text)  # добавляем строку параграфа
            substitution_values.append(
                change_numbers(replacement_positions,text_find,text_refind))  # добавляем кортеж с позициями замен
            list_of_changes.append(
                substitution_values)  # добавляем кортеж с текстом параграфа и вложеным кортежем с номерами замен


def change_numbers(list_mail,text_find,
                   text_refind):  # функция принимает список исходных замен и возвращает кортеж из кортежей замен от-до
...
def refind_in_table(doc_table,text_find,text_refind,
                    list_repl):  # поиск по таблице, с "ячейкой" работаем как с параграфом
