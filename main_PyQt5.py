...
class MyWin(QtWidgets.QMainWindow):
    def __init__(self,parent=None):
        ...
        self.ui.pushButton_browse.clicked.connect(
            self.BrowseDirectory)  # Здесь прописываем событие нажатия на кнопку Browse
        ...
        self.ui.pushButton_convert.clicked.connect(
            self.ConvertBotton)  # Здесь прописываем событие нажатия на кнопку Convert
    ...
    def BrowseDirectory(self):  # открывает окно выбора директории и работает с ним
        directory = QtWidgets.QFileDialog.getExistingDirectory(self,"выберите папку").replace("/",
                                                                                              "\\")  # записывает выбранный путь в переменную, получаем str и изменяем слеши
    ...
    def onChanged_dir(self):  # изменения списка файлов и вывод информации об этом
        ...
        self.mythread = MyThread_onChangel_dir(flag=self.ui.checkBox_subdir.checkState(),
                                               dir=frozen_dir)  # Создаем экземпляр класса потока для поиска файлов
        self.mythread.finished.connect(self.on_finished_thread_dir)  # выполняем действие когда  завершится поток
        self.mythread.mysignal.connect(self.on_change_dir,
        ...
        for row in range(self.rows):
            if self.lines[row][0].text():  # если искомое текст заполнен, то выполнять иначе закончить цикл
                if not self.lines[row][
                           0].text() in self.dict_find_refind.keys():  # Проверка значений чтобы небыло одинаковых замен
                    self.dict_find_refind[self.lines[row][0].text()] = self.lines[row][1].text()
                else:
                    self.error_The_replacement_text_is_repeated = The_replacement_text_is_repeated(
                        "The replacement text is repeated: \n {}".format(self.lines[row][0].text()))
                    self.error_The_replacement_text_is_repeated.show()
    ...
    def on_errorBrokenFile(self,attr_int):  # обработка ошибки битого файла
        ...
        if (attr_int + 1) == self.number_of_files:
            self.ui.progressBar.setValue(
                100)  # изменяем прогресс бар на последнем файле под 100%. Пропуск последнего битого файла
    ...
    def on_signal_start_return(self,
                               files_num_self):  # через ****, но работает. Сигнал начала/продолжения основной работы
...
<p>Не являюсь профессиональным программистом. Просьба присылать свои замечания и материалы как их исправить на почту <u><font color="blue">Adorari@mail.ru<font></u> или GitHub <u><font color="blue">https://github.com/Olimpirat</font></u> .</p>
...
class The_replacement_text_is_repeated(
    QtWidgets.QWidget):  # класс с окном для вывода ошибки и кнопкой OK (используется в одинаковых поисках текста, незавершенном поиске файлов и битом файле
    def __init__(self,text,parent=None):
        ...
        self.The_replacement_text_is_repeated.pushButton.clicked.connect(
            self.exit_)  # Здесь прописываем событие нажатия на кнопку OK
...
def recruiting_list(directory,
                    list_for_extend=[]):  # list_for_extend это список для добавления в него, по умолчанию новый список. Функция возвращает список файлов в поддиректориях
    list_directory = []
...
def get_list_files_in_folder(directory,
                             frozen_dir=''):  # получения списка файлов в одной папке, подставляя frozen_dir спереди имени файла.
    files_list = []
    for file in getting_dict_or_list(directory,folder=False,
                                     type_return='list'):  # тут получаем окончательный путь к файлу, типа string. 'H:\\работа\\копия\\Некрасовка съем\\Некрасовка Адреса 2018год.xlsx'
        if not "~$ " in file:  # проверка на временные файлы, временные файлы надо убрать
            files_list.append(frozen_dir + file)
    return files_list


def getting_dict_or_list(directory,files=True,folder=True,
                         type_return='dict'):  # получение пронумерованного словаря или списка файлов и директорий по флагам
